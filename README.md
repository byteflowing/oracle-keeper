...
| 维度 | 随机化 |
|------|--------|
| 轮次间隔 | 30–95 分钟均匀随机 + 12% 概率 8–20 分钟"短跟进"（序列有双峰和空洞，无固定周期） |
| 轮内延迟 | 触发后再随机 0–2 分钟 |
| 尖峰高度 | 每轮随机：占空比 65%–125%×配置值（钳制 10%–95%）、核数 50%–100% |
| 尖峰宽度 | 时长 60%–160%×配置值；30% 概率拆成两段 burst 中间歇 25%–100% 时长 |
| 尖峰质地 | 首尾对称爬坡（最长 75s）+ 顶部慢正弦波动（幅度 20%–40%，周期 45–120s），1 分钟粒度的监控上呈起伏丘陵而非平顶矩形 |
...
| `ORACLE_KEEPER_SCHEDULE_INTERVAL_MIN` | `30m` | 随机间隔下限 |
| `ORACLE_KEEPER_SCHEDULE_INTERVAL_MAX` | `95m` | 随机间隔上限（均值约 1h；另有 12% 概率 8–20 分钟短跟进，序列无固定周期） |
